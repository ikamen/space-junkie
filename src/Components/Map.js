...
export default function Map(props) {
  ...
  const handleSubmit = () => {
    const constellationFound = constellations.filter(
      (obj) => obj.label === props.constelation.label
    );
  ...
  return (
    <>
      ...
      <Select
        ...
        onChange={(e) =>
          props.setConstelation({ value: e.value, label: e.label })
        }
      />
      <DatePicker onChange={props.setDate} value={props.date} />
      <button onClick={handleSubmit}>
        {props.isOpen ? "Hide" : "Show"} map
      </button>
